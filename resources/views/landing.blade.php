        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="h-16 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <a href="/admin" class="text-sm font-semibold text-slate-700 hover:text-slate-900 transition">Login</a>
                    <a href="/admin" class="text-sm font-semibold px-4 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-100 transition">
                        Administração
                    </a>
                </div>
            </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                    ...
                    <div class="lg:col-span-7">
                        <p class="inline-flex items-center rounded-full bg-white border border-slate-200 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-slate-600">
                            Produto digital
                        </p>

                        <h1 class="mt-5 text-4xl sm:text-5xl lg:text-6xl font-black leading-tight tracking-tight text-slate-900">
                        ...
                        <section id="beneficios" class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl">
                            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Compra rápida e simples</div>
                            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Acesso imediato ao conteúdo</div>
                            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Pagamento seguro</div>
                            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Suporte por contato</div>
                        </section>
                    </div>
                    ...
                    <aside id="checkout" class="lg:col-span-5">
                        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                            ...
                            <form method="POST" action="/checkout/criar" class="mt-5 space-y-3">
                                @csrf
                                <input type="text" name="nome" placeholder="Seu nome" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300" required>
                                <input type="email" name="email" placeholder="seu@email.com" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300" required>
                                <button type="submit" class="w-full rounded-xl bg-slate-900 py-3 text-white font-bold hover:bg-slate-800 transition">
                                    Comprar agora
                                </button>
                            </form>
                        </div>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
                @if($conteudo->imagem ?? false)
                    <img src="/uploads/{{ $conteudo->imagem }}" alt="Imagem do produto" class="w-full rounded-xl object-cover max-h-[500px]">
                @else
                    <div class="h-72 sm:h-96 rounded-xl bg-gradient-to-br from-slate-100 to-sky-100 flex items-center justify-center text-slate-500">
                        Imagem do produto
                    </div>
                @endif
            </div>
